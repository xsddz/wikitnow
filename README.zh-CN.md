...
<p align="center">
  <strong>现在就把它 Wiki 化！🚀</strong> — 一个轻量级、高性能的命令行生产力工具，能够将您的本地 Markdown 笔记与目录结构极速转化为体系化的云端知识库。
</p>
...
在本地编写和整理文档，让 `wikitnow`（*wiki it now!*）替您干脏活累活。**这不仅是一个同步脚本，更是一个无缝连接本地脑图与广袤云端知识库的搭建桥梁。**

- 🏗️ **极速建库**: 一次命令，瞬间将您的本地网状目录树 1:1 映射构建为知识库的层级节点。
- 🖥️ **跨平台**: 提供 macOS, Linux, Windows 原生二进制文件。
- 📂 **智能分析**: 自动推导处理单文件发布，或整个复杂目录树的递归架构搭建。
- 🛡️ **Git 风格排除**: 支持 `.wikitnow/ignore`，语法完全兼容标准的 `.gitignore` Glob 匹配法则。
- ⚡ **开箱即用**: Go 语言编写，编译为单一无依赖二进制文件。
- 🔒 **默认安全**: 默认运行行为是安全的**只读预览**，必须追加 `--apply` 参数以确认执行云端发布，杜绝误操作产生大量脏页面。
- 🔌 **可扩展**: 基于 Provider 接口设计，当前支持飞书（Lark），架构上易于扩展至其他平台。
